...
{
    public partial class frmDirectoryTypeComponents : Form
    {
        public frmDirectoryTypeComponents()
        {
            InitializeComponent();
        ...
        {
            frmAddTypeComponents frm = new frmAddTypeComponents();
            frm.Text = "Добавить тип комплектующего";
        ...
        { if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
            frmAddTypeComponents frm = new frmAddTypeComponents();
            frm.Text = "Редактировать тип комплектующего";
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
                if (DialogResult.Yes == MessageBox.Show("Удалить запись?", "Запрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2))
                {
                    ...
                    DataTable dtResult = readSQL.setTypeComponents(id, cName, 2, true, "");
        ...
        {
            dtData = readSQL.getTypeComponents();
            filter();
